<?php if (isset($data)) : ?>
    ...
    $_save_label = se($data, "save_label", "Save", false);
    $_save_classes = se($data, "save_classes", "btn btn-info", false);
    $_unsave_url = se($data, "unsave_url", "", false);
    $_unsave_label = se($data, "unsave_label", "Unsave", false);
    $_unsave_classes = se($data, "unsave_classes", "btn btn-warning", false);
    ...
    // end edge case
    $_has_atleast_one_url = $_view_url || $_edit_url || $_delete_url || $_save_url || $_unsave_url || $_post_self_form;
                    ...
                    <tr>
                        ...
                        <?php if ($_has_atleast_one_url) : ?>
                            <td>
                                ...
                                <?php endif; ?>
                                <?php if ($_save_url) : ?>
                                    <a href="<?php se($_save_url); ?>?<?php se($_primary_key_column); ?>=<?php se($row, $_primary_key_column); ?>" class="<?php se($_save_classes); ?>"><?php se($_save_label); ?></a>
                                <?php endif; ?>
                                <?php if ($_unsave_url) : ?>
                                    <a href="<?php se($_unsave_url); ?>?<?php se($_primary_key_column); ?>=<?php se($row, $_primary_key_column); ?>" class="<?php se($_unsave_classes); ?>"><?php se($_unsave_label); ?></a>
                                <?php endif; ?>
