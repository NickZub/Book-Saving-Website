...
$query = "SELECT id, bookID, title, author, image, rating FROM `Books` WHERE id IN 
(SELECT bookID FROM `UserBooks` WHERE userID = :userID AND is_active=1)";//ORDER BY created DESC LIMIT 25
...
$stmt2 = $db->prepare("SELECT * FROM `UserBooks` WHERE userID = :userID AND is_active=1");
$results = [];
$count = 0;
$total = 0;
try {
    ...
    //Get total count associated to user before filter
    $stmt2->execute([":userID" => get_user_id()]);
    $r2 = $stmt2->fetchAll();
    if ($r2) {
        $total = count($r2);
    }
...
$table = ["data" => $results, "title" => "Personal Books", "ignored_columns" => ["id", "bookID"], "edit_url" => get_url("admin/edit_personal_book.php"), "view_url" => get_url("view_personal_book.php"), "unsave_url" => get_url("delete_personal_book.php")];
?>
<div class="container-fluid">
    ...
    <form method="GET">
        ...
        <a href="<?php echo get_url("delete_every_personal_book.php"); ?>" class="btn btn-danger">Unsave All</a>
    </form>
